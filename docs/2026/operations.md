...
| 10 | Print maps, price lists, timetables | Week 3 | Hard dependency on 6, 7 and 11. QR codes on each team's map point to `https://vmfan.github.io/kelingkong/index.html?tim=N` — **live and verified 2026-08-21** (see `../../web/README.md` Hosting section). Still needed: generate the 25 per-team QR images (N=1..25) for the print run |
...
| 24 | **Apply the widened mission window and retimed broadcast schedule to the live Sheet** | Week 3, before printing | **Closed 2026-08-20.** `Missions!deadline` is now `issued_at + 20 min` (all 100 pre-filled rows); verified live against the deployed endpoint at the 19-of-20-minute boundary, where the old `+15` formula would already have expired. The five retimed `issued_at` values (M1–M5: 10:59 / 13:44 / 14:29 / 14:59 / 15:29, all 2026-09-12) are pre-filled in `Missions`; `text` and `penalty_kd` are left for the committee, per `missions-build-spec.md` |
