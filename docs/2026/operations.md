...
| 6 | Write 51 on-site landmark tasks | Week 2 | Largest content job. **Closed 2026-09-09** — `task_description` column added to `landmarks.csv`, one per row, transcribed from `data/Landmark Guidebook KELINGKONG 2026.md`. Delivered to participants via the live web app instead of print (item 10's maps/price lists never got built): `harga!I` (`tugas`) carries the text, `web/index.html`'s `card()` renders it on each landmark's card. **Deliberately still in English** — CLAUDE.md requires participant-facing text to be Indonesian; shipped without a translation pass as a conscious call given the Sep 12 date, not an oversight. Revisit before event day if there's time |
...
| 10 | Print maps, price lists, timetables | Week 3 | Hard dependency on 6, 7 and 11. QR codes on each team's map point to `https://vmfan.github.io/kelingkong/index.html?tim=N&key=...` — **live and verified 2026-08-21** (see `../../web/README.md` Hosting section). `key` added 2026-08-31 to stop team-spoofed submissions (item 25); `TeamKeys` populated and team count fixed at 21 (item 26). **QR images generated 2026-09-01** — `print/qr/tim-01.png`…`tim-21.png`, built by `scripts/generate_team_qr.py` from `scripts/team-keys.csv` (a checked-in snapshot of the live, deployed `TeamKeys` values — not a fresh key regeneration). Ready to place on the printed maps; still needed: the maps/price lists/timetables themselves. **Member (view-only) link added 2026-09-09**, param discreetened 2026-09-09 — same team key, `&ref=<CONFIG.VIEW_TOKEN>` appended (an opaque token, not a self-explanatory `role=member`, so a member skimming the address bar doesn't immediately see what to delete — still just a speed bump against casual tampering, not real enforcement; `web/index.html`'s `CONFIG.VIEW_TOKEN` and `scripts/generate_team_qr.py`'s `VIEW_TOKEN` must stay in sync, same pattern as `Code.gs`'s `CONFIG.STAFF_KEY`). Reads to hide the Beli/Tugas/Objek/Misi buttons and show "Tampilan Anggota" instead (UI-only, not a new credential; see `ledger-system.md`'s trust model). Generate with `scripts/generate_team_qr.py --keys-csv scripts/team-keys.csv --role member`, which writes `print/qr/tim-NN-anggota.png` and never touches the already-printed leader `tim-NN.png` set. Since the map print run already happened, share these with members digitally (team chat / a shared doc) rather than reprinting maps |
