...
at up to 5 missions/day (2025's count) the worst-case exposure is 50 KD, a third of the 150
KD starting allowance — enough to be felt, not enough to end a team's day. It is stored per
mission row in the `Missions` tab (`penalty_kd`, see `ledger-system.md`), not as a single
board-wide constant, so the committee can vary it per mission or change it mid-event without
